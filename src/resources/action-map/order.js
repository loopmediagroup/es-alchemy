...
const actionMapFilter = require('./filter');
  ...
  asc: (l, mode = null, filter = null) => ({
    [l]: Object.assign(
      ...
      l.indexOf('.') === -1 ? {} : {
        nested: {
          path: l.substring(0, l.lastIndexOf('.')),
          filter: filter === null ? null : filter.map(f => actionMapFilter[f[1]](f[0], ...f.slice(2)))
        }
      }
    )
  }),
  desc: (l, mode = null, filter = null) => ({
    [l]: Object.assign(
      { order: 'desc' },
      mode !== null ? { mode } : {},
      l.indexOf('.') === -1 ? {} : {
